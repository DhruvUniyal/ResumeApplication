...
import com.example.resume.constants.TextAlignment;
...
//	private static float allowedPageHeight = maxPageHeight - 2 * margin;
	...
	private static void writeLine(String line, PDFont font, float fontSize, TextAlignment.ALIGNMENT alignment)
			throws Exception {
		try {
			logger.info("Text Alignment :: " + alignment);
			if (alignment.equals(TextAlignment.ALIGNMENT.JUSTIFY)) {
				writeJustifiedLines(line, font, fontSize);
			} else {
				float startingPosition = getStartingPosition(line, font, fontSize, alignment);
				logger.info("Starting Position :: " + startingPosition);
				contentStream.beginText();
				contentStream.setFont(font, fontSize);
				contentStream.newLineAtOffset(startingPosition, availablePageHeight);
	...
	private static float getStartingPosition(String line, PDFont font, float fontSize,
			TextAlignment.ALIGNMENT alignment) throws Exception {
		try {
			float textWidth = (font.getStringWidth(line) / 1000f) * fontSize;
			if (alignment.equals(TextAlignment.ALIGNMENT.CENTER)) {
				return margin + (availablePageWidth - textWidth) / 2;
			}
			if (alignment.equals(TextAlignment.ALIGNMENT.RIGHT)) {
				return margin + availablePageWidth - textWidth;
			}
			if (alignment.equals(TextAlignment.ALIGNMENT.LEFT)) {
				return margin;
			}
		} catch (Exception $ex) {
			throw new Exception("Cannot return starting position. \n" + $ex);
		}
		return margin;
	}

	private static void writeJustifiedLines(String line, PDFont font, float fontSize) throws Exception {
		logger.info("Rendering Justified Lines");

		String[] lines = line.trim().split("\\s+");
		int numGaps = lines.length;

		float textWidth = 0f;
		for (String word : lines) {
			textWidth += (font.getStringWidth(word) / 1000f) * fontSize;
		}


		if (numGaps == 1) {
			writeLine(line, font, fontSize, TextAlignment.ALIGNMENT.LEFT);
			return;
		}

		numGaps--;

		float spaceWidth = (font.getSpaceWidth() / 1000f) * fontSize;
		float remainingSpace = availablePageWidth - (textWidth + numGaps * spaceWidth);
		float spacePerGap = remainingSpace / numGaps;
		float newSpaceWidth = spacePerGap + spaceWidth;

		float horizontalTextLimit = margin;

		logger.info("Text space :: " + textWidth + ", Remaining space :: " + remainingSpace + ", Gaps :: " + numGaps
				+ ", Space per gap :: " + spacePerGap + ", New Space Width :: " + newSpaceWidth);

		contentStream.beginText();
		contentStream.setFont(font, fontSize);
		contentStream.newLineAtOffset(horizontalTextLimit, availablePageHeight);

		for (int i = 0; i <= numGaps; i++) {

			contentStream.setTextMatrix(Matrix.getTranslateInstance(horizontalTextLimit, availablePageHeight));
			contentStream.showText(lines[i]);

			float wordWidth = (font.getStringWidth(lines[i]) / 1000f) * fontSize;
			horizontalTextLimit += wordWidth + newSpaceWidth;
		}
	...
	private static void createSection(String text, PDFont font, int fontSize, TextAlignment.ALIGNMENT alignment)
			throws Exception {
		...
		for (String line : textLines) {
			...
			if(alignment.equals(TextAlignment.ALIGNMENT.JUSTIFY)) {
				for (int i = 0; i < lines.size(); i++) {
					...
					if(i < lines.size()-1) {
						writeJustifiedLines(lines.get(i), font, fontSize);
					}
					else {
						writeLine(lines.get(i), font, fontSize, TextAlignment.ALIGNMENT.LEFT);
					}
				}
			}
				...
				for (String wrappedLine : lines) {
					...
					writeLine(wrappedLine, font, fontSize, alignment);
				}
			}
		}
	}
	...
	public static void createDocument(String path) {
		try {
			...
			createSection(
					...
					generalFont, generalFontSize, TextAlignment.ALIGNMENT.CENTER);
			createSection("New Section added - 1", boldAccentFont, accentFontSize, TextAlignment.ALIGNMENT.LEFT);
			createSection(
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed blandit sed magna a ultrices. Suspendisse placerat tincidunt viverra. Duis commodo lacinia neque. Sed et molestie nibh, in vulputate dolor. Curabitur vulputate eu nisl mollis maximus. Sed non tellus quis tortor semper euismod sodales non nunc. Ut in orci scelerisque, fermentum magna vel, suscipit nulla. In ex neque, congue id egestas eu, blandit quis erat.\r\n"
							+ "\r\n"
							+ "Nunc et enim in ante dignissim fermentum id in lectus. Ut consectetur pulvinar ornare. Praesent quis felis ut ante blandit pretium. Curabitur porttitor finibus leo, quis elementum orci imperdiet et. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Ut feugiat elit quis lobortis tincidunt. Ut viverra sapien at nisi aliquam, eget eleifend nisi congue. Fusce elit elit, sodales et ornare id, suscipit sit amet libero.\r\n"
							...
							+ "\r\n"
							+ "Maecenas tincidunt magna sed odio placerat, ac placerat lorem elementum. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus dictum maximus molestie. Phasellus id turpis a risus hendrerit rhoncus eu in lectus. Nulla a pellentesque dolor. Phasellus aliquam ac arcu volutpat dictum. Nunc id augue vitae diam vulputate feugiat. Morbi tristique a ipsum in egestas. Nullam vestibulum lobortis mauris. Nullam sodales turpis ac iaculis venenatis. Cras in nisi nulla. Cras pellentesque augue ut magna dictum, vel euismod tellus gravida. Etiam viverra urna a dapibus commodo.",
					generalFont, generalFontSize, TextAlignment.ALIGNMENT.LEFT);

			createSection("New Section added - 2", headerFont, headerFontSize, TextAlignment.ALIGNMENT.RIGHT);
			createSection(
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed blandit sed magna a ultrices. Suspendisse placerat tincidunt viverra. Duis commodo lacinia neque. Sed et molestie nibh, in vulputate dolor. Curabitur vulputate eu nisl mollis maximus. Sed non tellus quis tortor semper euismod sodales non nunc. Ut in orci scelerisque, fermentum magna vel, suscipit nulla. In ex neque, congue id egestas eu, blandit quis erat.\r\n"
							+ "\r\n"
							+ "Nunc et enim in ante dignissim fermentum id in lectus. Ut consectetur pulvinar ornare. Praesent quis felis ut ante blandit pretium. Curabitur porttitor finibus leo, quis elementum orci imperdiet et. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Ut feugiat elit quis lobortis tincidunt. Ut viverra sapien at nisi aliquam, eget eleifend nisi congue. Fusce elit elit, sodales et ornare id, suscipit sit amet libero.\r\n"
							...
							+ "\r\n"
							+ "Maecenas tincidunt magna sed odio placerat, ac placerat lorem elementum. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus dictum maximus molestie. Phasellus id turpis a risus hendrerit rhoncus eu in lectus. Nulla a pellentesque dolor. Phasellus aliquam ac arcu volutpat dictum. Nunc id augue vitae diam vulputate feugiat. Morbi tristique a ipsum in egestas. Nullam vestibulum lobortis mauris. Nullam sodales turpis ac iaculis venenatis. Cras in nisi nulla. Cras pellentesque augue ut magna dictum, vel euismod tellus gravida. Etiam viverra urna a dapibus commodo.",
					accentFont, accentFontSize, TextAlignment.ALIGNMENT.RIGHT);
			createSection("New Section added - 3", headerFont, headerFontSize, TextAlignment.ALIGNMENT.JUSTIFY);
			createSection(
					"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed blandit sed magna a ultrices. Suspendisse placerat tincidunt viverra. Duis commodo lacinia neque. Sed et molestie nibh, in vulputate dolor. Curabitur vulputate eu nisl mollis maximus. Sed non tellus quis tortor semper euismod sodales non nunc. Ut in orci scelerisque, fermentum magna vel, suscipit nulla. In ex neque, congue id egestas eu, blandit quis erat.\r\n"
							+ "\r\n"
							+ "Nunc et enim in ante dignissim fermentum id in lectus. Ut consectetur pulvinar ornare. Praesent quis felis ut ante blandit pretium. Curabitur porttitor finibus leo, quis elementum orci imperdiet et. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Ut feugiat elit quis lobortis tincidunt. Ut viverra sapien at nisi aliquam, eget eleifend nisi congue. Fusce elit elit, sodales et ornare id, suscipit sit amet libero.\r\n"
							...
							+ "\r\n"
							+ "Maecenas tincidunt magna sed odio placerat, ac placerat lorem elementum. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus dictum maximus molestie. Phasellus id turpis a risus hendrerit rhoncus eu in lectus. Nulla a pellentesque dolor. Phasellus aliquam ac arcu volutpat dictum. Nunc id augue vitae diam vulputate feugiat. Morbi tristique a ipsum in egestas. Nullam vestibulum lobortis mauris. Nullam sodales turpis ac iaculis venenatis. Cras in nisi nulla. Cras pellentesque augue ut magna dictum, vel euismod tellus gravida. Etiam viverra urna a dapibus commodo.",
					accentFont, accentFontSize, TextAlignment.ALIGNMENT.JUSTIFY);
			close(path);
		} catch (Exception $ex) {
